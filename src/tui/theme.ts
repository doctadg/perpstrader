...
export function formatUSD(value: number): string {
  ...
  if (abs >= 1_000) return `${prefix}$${abs.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
...
export function pnlIcon(value: number): string {
  if (value > 0) return icons.up;
  if (value < 0) return icons.down;
