...
export async function rebalancePlannerNode(
  ...
  try {
    ...
    if (!trigger.shouldRebalance) {
      logger.info('[RebalancePlanner] No rebalance triggered');
      return {
    ...
    logger.info(
      `[RebalancePlanner] Generated ${actions.length} actions. ` +
    ...
    logger.error(`[RebalancePlanner] Failed: ${error}`);
    return {
 ...
 */
async function evaluateRebalanceTrigger(
  state: SafekeepingFundState
  ...
  if (aprDelta > DEFAULT_SAFETY_LIMITS.minAPRImprovement) {
    return {
 ...
 * Generate rebalance actions
 */
async function generateRebalanceActions(
  state: SafekeepingFundState
  ...
  if (state.bestOpportunity && state.positions.length > 0) {
    ...
    if (improvement > DEFAULT_SAFETY_LIMITS.minAPRImprovement) {
...
function findWorstPosition(
  ...
  return positions.reduce((worst, current) =>
    current.effectiveAPR < worst.effectiveAPR ? current : worst
  );
}
...
function selectBestAction(
  ...
  const validActions = actions.filter(action => {
    // Check size limits
    if (action.amount > DEFAULT_SAFETY_LIMITS.maxTradeSize) {
      logger.warn(`[RebalancePlanner] Action ${action.id} exceeds max trade size`);
      return false;
    }

    // Check if we have enough value
    if (action.amount > state.totalValue) {
      logger.warn(`[RebalancePlanner] Action ${action.id} exceeds total value`);
      return false;
    }
