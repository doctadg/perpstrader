...
import type { PoolOpportunity, APRBreakdown, LiquidityPosition } from '../types';

/**
 * APR Calculator Node
 * Processes raw pool data and calculates effective APRs
 */
export async function aprCalculatorNode(
  state: SafekeepingFundState
): Promise<Partial<SafekeepingFundState>> {
  logger.info('[APRCalculator] Calculating effective APRs for opportunities');
  ...
  try {
    ...
    logger.info(
      `[APRCalculator] Processed ${processedOpportunities.length} opportunities. ` +
    ...
    logger.error(`[APRCalculator] Failed: ${error}`);
    return {
 ...
 */
async function enhanceOpportunityAPR(
 ...
 */
function calculateCompositeScore(opportunity: PoolOpportunity): number {
  // Weight factors
 ...
 * Adjust risk score based on multiple factors
 */
function adjustRiskScore(opportunity: PoolOpportunity): number {
  let risk = opportunity.riskScore;

  // Increase risk for low liquidity
  if (opportunity.tvl < 50000) risk += 0.15;
  else if (opportunity.tvl < 200000) risk += 0.05;

  // Increase risk for very high APR (likely volatile or risky)
  if (opportunity.effectiveAPR > 100) risk += 0.1;
  else if (opportunity.effectiveAPR > 50) risk += 0.05;
 ...
 */
function calculateWeightedAverageAPR(positions: LiquidityPosition[]): number {
 ...
 */
export function createAPRBreakdown(
