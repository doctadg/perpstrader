...
export async function enhancedStoryClusterNode(state: any): Promise<Partial<EnhancedClusteringState>> {
    ...
    for (const article of filteredArticles) {
        ...
        try {
            ...
            entities = entityResult.entities as ExtractedEntity[];
            stats.entitiesExtracted += entities.length;
        } catch (e) {
            logger.warn(`[EnhancedClusterNode] Entity extraction failed for "${article.title.slice(0, 50)}":`, e);
        }
    ...
    // ============================================================
    logger.info('[EnhancedClusterNode] PHASE 5b: Singleton re-clustering pass...');
    ...
    if (assignedClusterId) {
        ...
        for (const entity of entities) {
            try {
                ...
                logger.warn(`[EnhancedClusterNode] Entity linking failed: ${entity.name} -> ${assignedClusterId.slice(0, 8)}`, e);
            }
        }
        ...
        for (const entity of entities) {
            try {
                ...
                logger.warn(`[EnhancedClusterNode] New cluster entity linking failed: ${entity.name} -> ${newClusterId.slice(0, 8)}`, e);
            }
        }
 ...
 */
async function createCrossCategoryLinks(_processedArticles: Array<{ article: NewsItem; aiLabel: any; entities: ExtractedEntity[] }>): Promise<void> {
    try {
        // Find cluster pairs that share entities across category boundaries
        const pairs = await storyClusterStoreEnhanced.getCrossCategoryEntityPairs(CLUSTER_MERGE_HOURS_THRESHOLD);

        if (pairs.length === 0) {
            logger.info('[EnhancedClusterNode] PHASE 4: No cross-category entity bridges found');
            return;
        }
        ...
        for (const pair of pairs) {
            ...
            await storyClusterStoreEnhanced.createCrossRef(
                pair.sourceClusterId,
                pair.targetClusterId,
                'RELATED',
                confidence
            );
            crossRefCount++;
        ...
        logger.error('[EnhancedClusterNode] PHASE 4 cross-category linking failed:', error);
    }
}
...
async function mergeSimilarClustersEnhanced(useVectorMode: boolean): Promise<{ mergedCount: number; targetCount: number }> {
    const hotClusters = await storyClusterStoreEnhanced.getHotClusters(500, CLUSTER_MERGE_HOURS_THRESHOLD);
...
async function mergeSingletonClusters(): Promise<{ mergedCount: number }> {
    ...
    const allClusters = await storyClusterStoreEnhanced.getHotClusters(500, CLUSTER_MERGE_HOURS_THRESHOLD);
    const mergeTargets = allClusters.filter(c => c.articleCount >= 2);

    if (mergeTargets.length === 0) {
        logger.info('[EnhancedClusterNode] PHASE 5b: No multi-article clusters to merge into');
        return { mergedCount: 0 };
    }

    logger.info(`[EnhancedClusterNode] PHASE 5b: Found ${singletons.length} singletons, ${mergeTargets.length} merge targets (inverted index mode)`);
    ...
    const targetData = mergeTargets.map((t, idx) => {
        ...
        for (const kw of kwSet) {
            if (!kwIndex.has(kw)) kwIndex.set(kw, new Set());
        ...
        if (!catIndex.has(cat)) catIndex.set(cat, new Set());
        catIndex.get(cat)!.add(idx);
        for (const w of words) {
            if (!topicWordIndex.has(w)) topicWordIndex.set(w, new Set());
    ...
    for (const singleton of singletons) {
        ...
        for (const idx of candidateIdx) {
            ...
            if (sharedEntities.length >= 1) {
                ...
                if (sCat === td.cluster.category) score += 0.05;
            } else {
                // Keyword overlap
        ...
        if (bestTarget && bestScore >= threshold) {
            if (!(await storyClusterStoreEnhanced.clusterExists(singleton.id)) ||
                !(await storyClusterStoreEnhanced.clusterExists(bestTarget.id))) {
                continue;
            }

            const result = await storyClusterStoreEnhanced.mergeClusters(bestTarget.id, singleton.id);
 ...
 */
function isEntityRelevantToCluster(
    entityName: string,
    normalizedEntity: string,
    clusterTopic: string,
    clusterKeywords: string[]
): boolean {
    if (!clusterTopic && (!clusterKeywords || clusterKeywords.length === 0)) return true; // No topic = no filter
    ...
    for (const word of entityWords) {
        if (topicLower.includes(word)) return true;
    }
...
function extractEntitiesFromTopicAndKeywords(topic: string, keywords: string[]): string[] {
...
async function calculateEnhancedSimilarityAsync(c1: any, c2: any): Promise<ClusterSimilarityResult> {
    ...
    let entities1 = await storyClusterStoreEnhanced.getClusterEntities(c1.id);
    let entities2 = await storyClusterStoreEnhanced.getClusterEntities(c2.id);
        ...
        if (similarArticles.length > 0) {
            ...
            for (const art of similarArticles) {
                try {
                    ...
                    logger.warn(`[EnhancedClusterNode] Fallback entity extraction failed for ${art.id.slice(0, 8)}`, e);
                }
            }
