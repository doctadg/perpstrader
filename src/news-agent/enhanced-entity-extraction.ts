...
class EnhancedEntityExtractor {
  ...
  private readonly LOW_CONFIDENCE = 0.40;

  // Cache for LLM responses
  private llmCache = new Map<string, LLMEntityResponse>();
  ...
  private readonly ENTITY_BLACKLIST = new Set([
    ...
    // === GENERIC FINANCIAL/NEWS TERMS (NOT real entities) ===
    ...
    'analysis', 'forecast', 'outlook', 'gain', 'loss', 'profit', 'revenue',
    'risk', 'volatility', 'liquid', 'supply', 'demand', 'capital', 'fund', 'funds',
    'portfolio', 'asset', 'assets', 'sector', 'industry', 'economy',
    'regulation', 'compliance', 'enforcement', 'penalty', 'fine',
    'blockchain', 'network', 'platform', 'exchange',
    'wallet', 'mining', 'staking', 'yield', 'apy', 'tvl',
    'tariff', 'tariffs', 'sanction', 'embargo', 'boycott',
    'hiring', 'jobs', 'employment', 'unemployment', 'wage', 'wages',
    'deflation', 'expansion', 'recovery', 'crisis',
    'government', 'parliament', 'democratic', 'republican',
    'conflict', 'peace', 'treaty', 'summit',
    ...
    'surge', 'crash', 'dip', 'dump', 'pump', 'rally',
   ...
   */
  private isGarbageEntity(name: string, type: string): boolean {
    ...
    for (const [type, patterns] of Object.entries(this.PATTERNS)) {
      for (const pattern of patterns) {
        ...
        for (const match of matches) {
          ...
          // Skip duplicates
          if (seen.has(`${type}:${normalized}`)) continue;
...
Return JSON only:
{
  ...
  "primaryEntity": "The main entity this article is about"
      ...
      let llmResponse: LLMEntityResponse;
      ...
      logger.debug(`[EntityExtraction] LLM extraction failed: ${error}`);
      return [];
    }
  }
   ...
   */
  async extractHybrid(title: string, content?: string, articleId?: string): Promise<EntityExtractionResult> {
    ...
    // Find primary entity
    const primaryEntity = merged.find(e =>
      ['TOKEN', 'ORGANIZATION', 'GOVERNMENT_BODY'].includes(e.type) &&
   ...
   */
  private calculateRegexConfidence(name: string, type: string): number {
    let confidence = 0.7; // Base confidence

    // Boost for well-known entities
    const wellKnownPatterns: Record<string, RegExp[]> = {
   ...
   */
  private inferEntityType(keyword: string): ExtractedEntity['type'] {
   ...
   */
  private cacheResponse(key: string, response: LLMEntityResponse): void {
  ...
  clearCache(): void {
    this.llmCache.clear();
    logger.info('[EntityExtraction] Cache cleared');
  }
...
export const enhancedEntityExtractor = new EnhancedEntityExtractor();
