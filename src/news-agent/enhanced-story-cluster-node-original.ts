...
export async function enhancedStoryClusterNode(state: any): Promise<Partial<EnhancedClusteringState>> {
    ...
    logger.info(`[EnhancedClusterNode] Processing ${filteredArticles.length} articles with enhanced clustering...`);
    ...
    // ============================================================
    logger.info('[EnhancedClusterNode] Batch labeling articles...');
    ...
    // ============================================================
    const processedArticles: Array<{ article: NewsItem; aiLabel: any; entities: ExtractedEntity[] }> = [];
    ...
    for (const article of filteredArticles) {
        ...
        if (aiLabel?.topic && aiLabel.topic.length > 5) {
            ...
            // Extract entities
            const entities = EntityExtractor.extractEntities(article.title, article.content || article.snippet || '');
    ...
    for (let i = 0; i < processedArticles.length; i += CLUSTER_BATCH_SIZE) {
        ...
        const batchResults = await Promise.all(
            batch.map(({ article, aiLabel, entities }) =>
        ...
        logger.info(`[EnhancedClusterNode] Progress: ${Math.min(i + CLUSTER_BATCH_SIZE, processedArticles.length)}/${processedArticles.length} (${stats.newClusters} new, ${stats.existingClusters} existing)`);
    }

    // ============================================================
    // PHASE 4: Cross-Category Linking
    // ============================================================
    logger.info('[EnhancedClusterNode] Creating cross-category links...');
    await createCrossCategoryLinks(processedArticles);

    // ============================================================
    // PHASE 5: Cluster Merging
    // ============================================================
    logger.info('[EnhancedClusterNode] Merging similar clusters...');
    ...
    // ============================================================
    logger.info('[EnhancedClusterNode] Recording heat history...');
    ...
    // ============================================================
    logger.info('[EnhancedClusterNode] Generating heat predictions...');
    ...
    // ============================================================
    logger.info('[EnhancedClusterNode] Calculating trending entities...');
    const trendingEntities = await storyClusterStoreEnhanced.getTrendingEntities(20, 24);

    // ============================================================
    // PHASE 9: Publish Events
    // ============================================================
    logger.info('[EnhancedClusterNode] Publishing events...');
    ...
    logger.info(`[EnhancedClusterNode] Clustering complete: ${stats.newClusters} new, ${stats.existingClusters} existing, ${stats.mergedClusters} merged, ${stats.anomaliesDetected} anomalies, ${stats.predictionsGenerated} predictions`);
...
async function processArticleEnhanced(
    ...
    if (!assignedClusterId) {
        const activeClusters = await storyClusterStoreEnhanced.getHotClusters(500, CLUSTER_MERGE_HOURS_THRESHOLD, article.categories[0]);
    ...
    if (assignedClusterId) {
        ...
        const result = await storyClusterStoreEnhanced.addArticleToCluster(
            assignedClusterId,
            article.id,
            titleFingerprint,
            heatDelta,
        ...
        for (const entity of entities) {
            const entityId = await storyClusterStoreEnhanced.findOrCreateEntity(entity.name, entity.type);
