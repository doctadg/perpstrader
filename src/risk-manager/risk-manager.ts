...
export class RiskManager {
  ...
  private trailingStopPct: number = 0.015;  // 1.5% trailing stop
  ...
  private calculatePositionSize(signal: TradingSignal, portfolio: Portfolio): number {
    ...
    logger.info(`[RiskManager] Size calc (40x): price=$${price.toFixed(2)}, available=$${portfolio.availableBalance.toFixed(2)}`);
    logger.info(`[RiskManager]   → confidence=${signal.confidence.toFixed(2)}, targetMargin=${(targetMarginPercent * 100).toFixed(1)}%`);
