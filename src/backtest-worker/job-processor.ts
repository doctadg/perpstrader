...
import { BacktestEngine, BacktestConfig } from '../backtest/enhanced-backtest';
import { Strategy, MarketData, BacktestResult } from '../shared/types';
import logger from '../shared/logger';
import BetterSqlite3 from 'better-sqlite3';
import { assessStrategy, StrategyAssessment } from './result-analyzer';
...
async function fetchHistoricalData(
  ...
  logger.info(`[JobProcessor] Fetching ${days} days of data for ${symbol} (${timeframe})`);
  ...
  try {
    ...
    try {
      ...
      rows = stmt.all(cutoffTimestamp) as CandleRow[];
    } catch (error) {
      // Table might not exist, try generic candles table
      logger.warn(`[JobProcessor] Table ${tableName} not found, trying generic candles table`);
      ...
      rows = stmt.all(symbol, timeframe, cutoffTimestamp) as CandleRow[];
    }
    
    db.close();
    
    if (rows.length === 0) {
      logger.warn(`[JobProcessor] No historical data found for ${symbol} (${timeframe})`);
      return [];
    }
    
    // Convert to MarketData format
    const candles: MarketData[] = rows.map(row => ({
      symbol,
    ...
    logger.info(`[JobProcessor] Fetched ${candles.length} candles for ${symbol}`);
    return candles;
...
async function fetchFromCache(
  ...
  logger.debug(`[JobProcessor] Cache fallback not implemented for ${symbol}`);
  return [];
}
...
export async function processBacktestJob(job: Job<BacktestJobData>): Promise<BacktestJobResult> {
  ...
  logger.info(`[JobProcessor] Processing backtest job ${job.id} for strategy ${strategy.id} on ${symbol}`);
  ...
  try {
    ...
    if (candles.length < 30) {
      logger.warn(`[JobProcessor] Limited data available: ${candles.length} candles for ${symbol}`);
    }
    ...
    const assessment = assessStrategy(result, strategy);
    ...
    logger.info(`[JobProcessor] Completed backtest job ${job.id} in ${processingTimeMs}ms:`, {
    ...
    logger.error(`[JobProcessor] Failed to process backtest job ${job.id}:`, error);
...
async function storeBacktestResult(
  ...
  try {
    ...
    logger.debug(`[JobProcessor] Stored backtest result for job ${jobId}`);
    
  } catch (error) {
    logger.error(`[JobProcessor] Failed to store backtest result:`, error);
  } finally {
    db.close();
  }
}
...
async function updateStrategyStatus(
  ...
  try {
    ...
    if (!tableCheck) {
      logger.warn(`[JobProcessor] Strategies table does not exist, skipping status update`);
      return;
    }
    ...
    const result = stmt.run(
      assessment.shouldActivate ? 1 : 0,
      assessment.metrics.totalTrades,
      Math.round(assessment.metrics.totalTrades * (assessment.metrics.winRate / 100)),
      Math.round(assessment.metrics.totalTrades * (1 - assessment.metrics.winRate / 100)),
      assessment.metrics.winRate,
      assessment.metrics.totalReturn,
      assessment.metrics.sharpeRatio,
      assessment.metrics.maxDrawdown,
      assessment.metrics.averageWin,
      assessment.metrics.averageLoss,
      assessment.metrics.profitFactor,
      new Date().toISOString(),
      strategyId
    );
    ...
    if (result.changes > 0) {
      logger.info(`[JobProcessor] Updated strategy ${strategyId}: active=${assessment.shouldActivate}, tier=${assessment.performanceTier}`);
    } else {
      logger.warn(`[JobProcessor] Strategy ${strategyId} not found in database`);
    }
    
  } catch (error) {
    logger.error(`[JobProcessor] Failed to update strategy status:`, error);
  } finally {
    db.close();
  }
}
