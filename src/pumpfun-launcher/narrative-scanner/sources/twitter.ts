...
const log = pino({ name: 'narrative-scanner:twitter' });
...
// ─── Twitter Source ──────────────────────────────────────────────
export class TwitterSource implements NarrativeSource {
  readonly name = 'twitter' as const;
  private client: AxiosInstance;
  private config: TwitterSourceConfig;
  ...
  private async searchTweets(query: string, count: number): Promise<ParsedTweet[]> {
    ...
    const resp = await this.client.get(
      `/i/api/graphql/${GRAPHQL_IDS.SearchTimeline}/SearchTimeline?${params}`,
    );
  ...
  // ─── UserTweets GraphQL ─────────────────────────────────
  private async fetchUserTweets(screenName: string, count: number): Promise<ParsedTweet[]> {
    ...
    const resp = await this.client.get(
      `/i/api/graphql/${GRAPHQL_IDS.UserTweets}/UserTweets?${params}`,
    );

    const timeline =
      resp.data?.data?.user?.result?.timeline_v2?.timeline ??
  ...
  private async resolveUserId(screenName: string): Promise<string | null> {
    try {
      ...
      const resp = await this.client.get(
        `/i/api/graphql/sLVLhk0bGj3MVFEKTdax1w/UserByScreenName?${params}`,
      );
  ...
  private extractTweets(instructions: any[]): ParsedTweet[] {
  ...
  private parseTweetEntry(entry: any): ParsedTweet | null {
    ...
    return {
      ...
      hashtags: (legacy.entities?.hashtags ?? []).map((h: any) => h.text.toLowerCase()),
    };
  }

  // ─── Convert to Narrative ───────────────────────────────
  private toNarrative(tweet: ParsedTweet): Narrative {
    const id = `twitter-${crypto.hash('sha256', tweet.id).slice(0, 12)}`;
    return {
      id,
      source: 'twitter',
      text: tweet.text,
