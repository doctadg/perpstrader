 ...
 */
export function validateRSIData(rsi: number[]): { valid: boolean; issues: string[] } {
    const issues: string[] = [];
 ...
 */
export function validateCandle(candle: { time: any, open: number, high: number, low: number, close: number, volume: number }): boolean {
    // Check for zero or negative prices
    if (candle.open <= 0 || candle.high <= 0 || candle.low <= 0 || candle.close <= 0) {
      logger.warn(`Invalid candle prices for ${candle.time}: open=${candle.open}, high=${candle.high}, low=${candle.low}, close=${candle.close}`);
      return false;
    }
    
    // Check for reasonable high/low relationship
    if (candle.high < candle.low) {
      logger.warn(`Invalid candle high/low: high=${candle.high} < low=${candle.low}`);
      return false;
    }
    
    // Check close is within high/low bounds
    if (candle.close < candle.low || candle.close > candle.high) {
      logger.warn(`Close outside high/low bounds: ${candle.close} not in [${candle.low}, ${candle.high}]`);
      return false;
    }
