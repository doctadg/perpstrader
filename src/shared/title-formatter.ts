 ...
 */
export function getAssetTicker(name: string): string {
  if (!name) return '';
 ...
 */
export function normalizePunctuation(title: string): string {
  return title
 ...
 */
export function removeTrailingAttribution(title: string): string {
  return title
 ...
 */
export function calculateTitleMetrics(title: string): TitleMetrics {
  ...
  if (words.length >= 4 && words.length <= 12) score += 1;
 ...
 */
export function getTitleQualityLabel(metrics: TitleMetrics): string {
 ...
 */
export function detectActionWord(title: string): string | null {
  const lowerTitle = title.toLowerCase();
 ...
 */
export function detectTrendDirection(title: string): 'UP' | 'DOWN' | 'NEUTRAL' {
  const lowerTitle = title.toLowerCase();
 ...
 */
export function detectEventType(title: string): SubEventType {
  const lowerTitle = title.toLowerCase();

  const patterns: Record<SubEventType, RegExp[]> = {
  ...
  for (const [eventType, regexes] of Object.entries(patterns)) {
    for (const regex of regexes) {
      if (regex.test(lowerTitle)) {
        return eventType as SubEventType;
      }
    }
  }
