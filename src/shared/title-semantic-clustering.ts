...
class TitleSemanticClustering {
  ...
  private readonly SEMANTIC_MATCH_THRESHOLD = 0.80;
  private readonly ENTITY_MATCH_THRESHOLD = 0.70;
  private readonly TOPIC_MATCH_THRESHOLD = 0.60;
  private readonly MAX_BATCH_SIZE = 50;

  // Cache for title analyses
  private analysisCache = new Map<string, TitleAnalysis>();
  private cacheMaxSize = 1000;

  /**
   * Calculate similarity between two titles using multiple methods
   */
  async calculateSimilarity(title1: string, title2: string): Promise<TitleSimilarityResult> {
    ...
    const sharedEntities = analysis1.entities.filter(e =>
      analysis2.entities.some(e2 => e.toLowerCase() === e2.toLowerCase())
    );
    ...
    const similarityScore = Math.min(1.0,
      entitySim * 0.4 +      // 40% entity overlap
      topicSim * 0.35 +      // 35% topic similarity
      phraseSim * 0.25       // 25% phrase overlap
    );

    // Determine similarity type
    let similarityType: TitleSimilarityResult['similarityType'] = 'none';
    if (similarityScore >= this.EXACT_MATCH_THRESHOLD) similarityType = 'exact';
    else if (similarityScore >= this.SEMANTIC_MATCH_THRESHOLD) similarityType = 'semantic';
    else if (similarityScore >= this.ENTITY_MATCH_THRESHOLD) similarityType = 'entity';
    else if (similarityScore >= this.TOPIC_MATCH_THRESHOLD) similarityType = 'topic';
  ...
  async clusterTitles(
  ...
  ): Promise<TitleCluster[]> {
    if (articles.length === 0) return [];

    logger.info(`[TitleClustering] Clustering ${articles.length} articles...`);
    ...
    for (const article of articles) {
      ...
      let bestCluster: TitleCluster | null = null;
      let bestScore = 0;
    ...
    logger.info(`[TitleClustering] Created ${clusters.length} clusters from ${articles.length} articles`);
    return clusters;
  }
   ...
   */
  private async getTitleAnalysis(title: string): Promise<TitleAnalysis> {
   ...
   */
  private async batchAnalyzeTitles(titles: string[]): Promise<TitleAnalysis[]> {
    // Process in parallel with concurrency limit
    const results: TitleAnalysis[] = [];
    const concurrency = 10;

    for (let i = 0; i < titles.length; i += concurrency) {
      const batch = titles.slice(i, i + concurrency);
      const batchResults = await Promise.all(
        batch.map(t => this.getTitleAnalysis(t))
      );
   ...
   */
  private extractEntitiesLocal(title: string): string[] {
   ...
   */
  private detectSentiment(title: string): TitleAnalysis['sentiment'] {
   ...
   */
  private calculateClusterSimilarity(analysis1: TitleAnalysis, analysis2: TitleAnalysis): number {
    const entitySim = this.calculateEntitySimilarity(analysis1.entities, analysis2.entities);
    const topicSim = this.calculateTopicSimilarity(analysis1.topic, analysis2.topic);
    const phraseSim = this.calculatePhraseOverlap(analysis1.keyPhrases, analysis2.keyPhrases);
  ...
  private calculateConfidence(
    analysis1: TitleAnalysis,
    analysis2: TitleAnalysis,
  ...
  clearCache(): void {
    ...
    logger.info('[TitleClustering] Analysis cache cleared');
  }
...
export const titleSemanticClustering = new TitleSemanticClustering();
