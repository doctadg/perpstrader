...
function extractTemplateVariables(asset, assetSymbol, entities, reason, authority, protocol, company) {
    var priceEntity = entities.find(function (e) { return e.type === 'price'; });
    var percentageEntity = entities.find(function (e) { return e.type === 'percentage'; });
    var amountEntity = entities.find(function (e) { return e.type === 'amount' || e.type === 'volume'; });
    return {
        asset: asset,
        assetSymbol: assetSymbol || asset,
        price: priceEntity ? formatPriceValue(priceEntity) : undefined,
        percentage: percentageEntity ? "".concat(percentageEntity.value, "%") : undefined,
...
function formatPriceValue(entity) {
    var value = entity.value, currency = entity.currency;
    if (currency === 'USD' || currency === 'EUR') {
        if (value >= 1000) {
            return "".concat(value.toLocaleString('en-US', { maximumFractionDigits: 0 }));
        }
...
function formatAmountValue(entity) {
    ...
    if (currency) {
        return "".concat(formatMagnitude(value), " ").concat(currency);
    }
    if (unit) {
