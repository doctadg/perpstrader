 ...
 */
export function calculateQualityScore(title: string, content?: string): number {
...
function evaluateTitleQuality(title: string): number {
  ...
  const capitalizedWords = words.filter((w, i) =>
    i > 0 && /^[A-Z][a-z]/.test(w) && !/^(The|A|An|This|That|These|Those)$/.test(w)
  );
 ...
 */
function evaluateContentQuality(title: string, content: string): number {
  let score = 0.5;
 ...
 */
function evaluateSourceQuality(title: string): number {
  // Negative indicators
 ...
 */
export function cleanTitle(title: string): { title: string; qualityScore: number } {
