 ...
 */
export function formatHumanReadableTitle(topic: string, articleTitle?: string): string {
 ...
 */
function fixGrammarPatterns(text: string): string {
 ...
 */
export function handleNonEnglishTitle(title: string, articleTitle?: string): string {
 ...
 */
export function validateAndFormatTopic(topic: string, articleTitle?: string): string {
  if (!topic || topic.length < 3) {
    return articleTitle || 'Market News';
  }
