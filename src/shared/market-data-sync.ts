...
class MarketDataSync {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      logger.info('[MarketDataSync] Initialized successfully');
    } catch (error) {
      logger.error('[MarketDataSync] Initialization failed:', error);
      throw error;
    }
  }
   ...
   */
  async fetchHyperliquidMarkets(): Promise<MarketData[]> {
    try {
      logger.info('[MarketDataSync] Fetching Hyperliquid market data...');
      ...
      const meta = await metaRes.json() as HyperliquidMeta;
      const context = await contextRes.json() as HyperliquidContext;
      ...
      const markets: MarketData[] = meta.universe
        .map((asset, index) => {
          ...
          if (!ctx || ctx.coin !== asset.name) {
            logger.warn(`[MarketDataSync] Mismatch at index ${index}: ${asset.name} vs ${ctx?.coin}`);
            return null;
          }
      ...
      logger.info(`[MarketDataSync] Fetched ${markets.length} Hyperliquid markets`);
      return markets;
    } catch (error) {
      logger.error('[MarketDataSync] Failed to fetch Hyperliquid markets:', error);
      return [];
    }
  }
   ...
   */
  async fetchPolymarketMarkets(): Promise<MarketData[]> {
    try {
      logger.info('[MarketDataSync] Fetching Polymarket data...');
      ...
      const marketData: MarketData[] = activeMarkets.map(m => {
      ...
      logger.info(`[MarketDataSync] Fetched ${marketData.length} Polymarket markets`);
      return marketData;
    } catch (error) {
      logger.error('[MarketDataSync] Failed to fetch Polymarket markets:', error);
      return [];
    }
  }
   ...
   */
  async syncAllMarkets(): Promise<{ hyperliquid: number; polymarket: number; total: number }> {
    ...
    try {
      ...
      logger.info(`[MarketDataSync] Synced ${result.hlCount} HL + ${result.pmCount} PM markets`);
      ...
      logger.error('[MarketDataSync] Sync failed:', error);
      throw error;
    }
  }
   ...
   */
  async deactivateStaleMarkets(hours: number = 24): Promise<number> {
    await this.initialize();
    if (!this.db) return 0;
    ...
    try {
      ...
      if (result.changes > 0) {
        logger.info(`[MarketDataSync] Deactivated ${result.changes} stale markets`);
      }
      return result.changes;
    } catch (error) {
      logger.error('[MarketDataSync] Failed to deactivate stale markets:', error);
      return 0;
    }
  }
  ...
  async getActiveMarkets(): Promise<MarketData[]> {
    ...
    try {
      ...
      logger.error('[MarketDataSync] Failed to get active markets:', error);
      return [];
    }
  }
  ...
  private categorizeCrypto(coin: string): string {
    const coinLower = coin.toLowerCase();
    
    // Major coins
    if (['btc', 'eth'].includes(coinLower)) return 'Layer 1';
    
    // L2s
    if (['arb', 'op', 'base', 'mnt', 'strk', 'zk'].includes(coinLower)) return 'Layer 2';
  ...
  private categorizePolymarket(market: PolymarketMarket): { category: string; subCategory: string } {
  ...
  private extractPoliticalSubcategory(question: string): string {
    if (question.includes('trump')) return 'Trump';
    if (question.includes('biden')) return 'Biden';
    if (question.includes('election')) return 'Elections';
    if (question.includes('congress') || question.includes('senate') || question.includes('house')) {
      return 'Congress';
    }
    if (question.includes('uk ') || question.includes('britain')) return 'UK';
    if (question.includes('france') || question.includes('macron')) return 'France';
    if (question.includes('germany')) return 'Germany';
    return 'General';
  }

  private extractSportsSubcategory(question: string): string {
    ...
    if (question.includes('tennis')) return 'Tennis';
    return 'General';
  }

  private calculatePriority(volume: number, type: 'hyperliquid' | 'polymarket'): number {
    if (type === 'hyperliquid') {
      // Priority based on volume tiers
      if (volume > 1_000_000_000) return 100; // $1B+
      ...
      if (volume > 100_000_000) return 95;
      if (volume > 50_000_000) return 85;
      if (volume > 10_000_000) return 75;
      if (volume > 1_000_000) return 65;
      if (volume > 100_000) return 50;
      return 35;
    }
  }

  private generateKeywords(market: MarketData): Array<{ keyword: string; type: string; weight: number }> {
    const keywords: Array<{ keyword: string; type: string; weight: number }> = [];
...
export const marketDataSync = new MarketDataSync();
