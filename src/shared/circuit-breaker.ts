...
interface CircuitBreakerState {
    ...
    critical: boolean; // If true, open state makes system CRITICAL and blocks trading
}
...
export class SafetyMonitor {
    ...
    canEnterNewTrade(symbol: string): boolean {
        ...
        if (BLACKLISTED_SYMBOLS.includes(normalizedSymbol)) {
            logger.debug(`[SafetyMonitor] Blocked trade for ${normalizedSymbol}: symbol is blacklisted`);
            return false;
        }
...
export class CircuitBreakerSystem {
     ...
     */
    registerBreaker(name: string, config: { threshold: number; timeout: number; critical?: boolean }): void {
