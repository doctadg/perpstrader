...
import { PatternMatch } from '../state';
 ...
 */
export async function patternRecallNode(state: AgentState): Promise<Partial<AgentState>> {
    logger.info(`[PatternRecallNode] Searching for similar patterns for ${state.symbol} ${state.timeframe}`);
    ...
    try {
        ...
        if (!state.candles || state.candles.length < 20) {
            logger.warn('[PatternRecallNode] Insufficient candles for pattern matching');
            return {
        ...
        if (!state.indicators) {
            logger.warn('[PatternRecallNode] No indicators available for pattern matching');
            return {
        ...
        const similarPatterns = await vectorStore.querySimilarPatterns(
            state.symbol,
            state.timeframe,
            state.candles,
            state.indicators,
        ...
        let patternBias: 'BULLISH' | 'BEARISH' | 'NEUTRAL' | 'MIXED' = 'NEUTRAL';
        ...
        logger.info(`[PatternRecallNode] Found ${similarPatterns.length} patterns, bias: ${patternBias}`);
        ...
        logger.error('[PatternRecallNode] Pattern recall failed:', error);
