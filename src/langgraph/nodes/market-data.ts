...
const lastProcessedSignature: Map<string, string> = new Map();
...
function toHyperliquidInterval(timeframe: string): string {
    ...
    const value = Number(match[1]);
    const unit = match[2].toLowerCase();
    if (unit === 's') return '1m'; // Hyperliquid doesn't support seconds, use 1m
 ...
 */
async function fetchHyperliquidCandles(symbol: string, timeframe: string, count: number): Promise<MarketData[]> {
    ...
    try {
        ...
        logger.warn(`[MarketDataNode] Failed to fetch candles from Hyperliquid API: ${error}`);
        return [];
    }
}
 ...
 */
export async function marketDataNode(state: AgentState): Promise<Partial<AgentState>> {
    logger.info(`[MarketDataNode] Fetching data for ${state.symbol} ${state.timeframe}`);
    ...
    try {
        ...
        let candles = await dataManager.getMarketData(
            state.symbol,
            startTime,
            endTime,
            limit
        ...
        if (candles.length < MIN_BACKTEST_CANDLES) {
            logger.warn(`[MarketDataNode] Only ${candles.length} candles in local DB, fetching all available`);
            candles = await dataManager.getMarketData(state.symbol, undefined, undefined, Math.max(limit, 2000));
        }

        // If still not enough data, fetch from Hyperliquid API directly
        if (candles.length < MIN_CANDLES) {
            logger.info(`[MarketDataNode] Fetching historical candles from Hyperliquid API for ${state.symbol}`);
            ...
            if (apiCandles.length >= MIN_CANDLES) {
                logger.info(`[MarketDataNode] Fetched ${apiCandles.length} candles from Hyperliquid API`);
                candles = apiCandles;
        ...
        if (isStale) {
            logger.info(`[MarketDataNode] Candle unchanged for ${state.symbol}, but continuing with indicator computation`);
        }

        // Compute technical indicators
        let indicators: NonNullable<AgentState['indicators']>;
        ...
        if (pool) {
            try {
                ...
                logger.warn('[MarketDataNode] Worker TA analysis failed, falling back to main thread:', error);
                indicators = await taModule.analyzeMarket(state.symbol, state.timeframe, candles);
            }
        } else {
            indicators = await taModule.analyzeMarket(state.symbol, state.timeframe, candles);
        }
        ...
        try {
            portfolio = await executionEngine.getPortfolio();
        } catch (error) {
            logger.warn('[MarketDataNode] Failed to get portfolio, using default');
            portfolio = {
                totalValue: 10000,
        ...
        logger.info(`[MarketDataNode] Loaded ${candles.length} candles, regime: ${regime}`);
        ...
        logger.error('[MarketDataNode] Failed to fetch market data:', error);
        return {
...
function detectMarketRegime(
    candles: AgentState['candles'],
    indicators: NonNullable<AgentState['indicators']>
): AgentState['regime'] {
