...
export async function strategyIdeationNode(state: AgentState): Promise<Partial<AgentState>> {
  logger.info(`[StrategyIdeationNode] Generating strategy ideas for ${state.symbol}`);
  ...
  try {
    const strategies = await glmService.generateTradingStrategies(researchData);
    ...
    logger.error('[StrategyIdeationNode] Strategy ideation failed:', error);
    const fallbackIdeas = generateFallbackIdeas(state);
    return {
      currentStep: 'STRATEGY_IDEATION_FALLBACK',
      strategyIdeas: fallbackIdeas,
...
function mapStrategiesToIdeas(strategies: Strategy[], state: AgentState): StrategyIdea[] {
...
function normalizeParameters(parameters: Record<string, any>): Record<string, number> {
...
function generateFallbackIdeas(state: AgentState): StrategyIdea[] {
  const baseRisk = {
...
export default strategyIdeationNode;
