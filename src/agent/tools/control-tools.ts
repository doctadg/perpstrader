  ...
  return new DynamicTool({
    ...
    func: async (input: string) => {
      ...
      try {
        ...
        logger.error(`[ControlTool] ${def.name} error:`, error);
 ...
 * Execute a trade
 */
const executeTrade: ToolDefinition = {
  name: 'execute_trade',
  ...
  handler: async ({ symbol, side, size, type = 'MARKET', price }: {
    ...
    // Publish event
    await messageBus.publish(Channel.EXECUTION_SUBMIT, {
      tradeId: result.id,
      source: 'conversational-agent',
      timestamp: new Date(),
    });

    logger.info(`[ControlTool] Trade executed: ${normalizedSide} ${size} ${normalizedSymbol}`);
 ...
 */
const updateRiskParams: ToolDefinition = {
  ...
  handler: async ({ maxPositionSize, maxDailyLoss, maxLeverage }: {
    ...
    logger.info('[ControlTool] Risk parameters updated:', updates);
 ...
 */
const emergencyStop: ToolDefinition = {
  name: 'emergency_stop',
  ...
  handler: async () => {
    ...
    logger.warn('[ControlTool] EMERGENCY STOP triggered via conversational agent');
 ...
 */
const resetCircuitBreaker: ToolDefinition = {
  ...
  handler: async ({ name }: { name: string }) => {
    ...
    if (success) {
      logger.info(`[ControlTool] Circuit breaker ${name} reset`);
      await messageBus.publish(Channel.CIRCUIT_BREAKER_CLOSED, {
        name,
        timestamp: new Date(),
      });
    }
 ...
 */
const startTrading: ToolDefinition = {
  name: 'start_trading',
  ...
  requiresConfirmation: true,
  handler: async () => {
    logger.info('[ControlTool] Starting autonomous trading via conversational agent');
 ...
 */
const stopTrading: ToolDefinition = {
  name: 'stop_trading',
  ...
  requiresConfirmation: true,
  handler: async () => {
    logger.info('[ControlTool] Stopping autonomous trading via conversational agent');
 ...
 */
const toggleStrategy: ToolDefinition = {
  ...
  riskLevel: 'MEDIUM',
  requiresConfirmation: true,
  handler: async ({ strategyId, isActive }: { strategyId: string; isActive: boolean }) => {
    if (!strategyId) {
    ...
    logger.info(`[ControlTool] Strategy ${strategyId} ${isActive ? 'enabled' : 'disabled'}`);
 ...
 */
const diagnoseIssue: ToolDefinition = {
  name: 'diagnose_issue',
  ...
  handler: async ({ component }: { component: string }) => {
    ...
    switch (component) {
      ...
      case 'data': {
        ...
        const recentTraces = traceStore.getRecentTraceSummaries(10);
      ...
      case 'circuit_breakers': {
        ...
        const tripped = Object.entries(breakers).filter(([, v]: any) => v.state === 'OPEN');
...
const CONTROL_TOOLS: Record<string, ToolDefinition> = {
 ...
 */
export function getControlTools(): DynamicTool[] {
 ...
 */
export function getControlToolDefinitions(): Record<string, ToolDefinition> {
