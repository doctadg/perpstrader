...
export class TAModule {
  ...
  private calculateMACD(prices: number[], fastPeriod: number = 12, slowPeriod: number = 26, signalPeriod: number = 9) {
    ...
    try {
      ...
      const validMacdData = macdData.filter(d =>
      ...
      const macdValues: number[] = validMacdData.map(d => d.MACD as number);
      const signalValues: number[] = validMacdData.map(d => d.signal as number);
      const histogramValues: number[] = validMacdData.map(d => d.histogram as number);
  ...
  private calculateStandardDeviation(prices: number[], period: number = 20): number[] {
    ...
    if (period <= 0 || prices.length === 0) return stdDev;
