  ...
  private parseStrategyResponse(response: string, marketRegime: MarketRegime): StrategyIdea[] {
    try {
      ...
      try {
        ...
        logger.warn('[StrategyGenerator] Malformed JSON from GLM, attempting repair');
        ...
        try {
          ...
          if (strategyObjects.length > 0) {
            parsed = { strategies: strategyObjects.map(s => { try { return JSON.parse(s); } catch { return null; } }).filter(Boolean) };
          } else {
            return [];
          }
        }
