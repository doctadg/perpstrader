...
function parseWinRate(value: string | undefined, fallback: number): number {
  if (!value) return fallback;
  const parsed = Number.parseFloat(value);
...
export function loadResearchEngineConfig(env: NodeJS.ProcessEnv = process.env): ResearchEngineConfig {
  const researchIntervalMs = parsePositiveInt(
    env.RESEARCH_INTERVAL_MS,
    parsePositiveInt(env.RESEARCH_INTERVAL_MINUTES, 15) * 60 * 1000
  );

  const evolutionIntervalMs = parsePositiveInt(
    env.EVOLUTION_INTERVAL_MS,
    parsePositiveInt(env.EVOLUTION_INTERVAL_HOURS, 6) * 60 * 60 * 1000
  );
  ...
  return {
    ...
    performanceThresholds: {
      minSharpeRatio: parsePositiveNumber(
        env.RESEARCH_MIN_SHARPE_RATIO,
        DEFAULT_MIN_SHARPE_RATIO
      ),
      minWinRate: parseWinRate(env.RESEARCH_MIN_WIN_RATE, DEFAULT_MIN_WIN_RATE),
    },
    maxConcurrentBacktests: parsePositiveInt(
      env.RESEARCH_MAX_CONCURRENT_BACKTESTS,
      DEFAULT_MAX_CONCURRENT_BACKTESTS
    ),
  };
}

export const researchEngineConfig = loadResearchEngineConfig();
