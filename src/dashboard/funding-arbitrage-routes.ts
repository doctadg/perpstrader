...
import hyperliquidAllMarkets from '../market-ingester/hyperliquid-all-markets';
...
router.get('/rates', async (req, res) => {
  try {
    ...
    let rates = await fundingArbitrageScanner.getAllCurrentRates();
    
    // Apply filters
    if (minApr !== undefined) {
      rates = rates.filter(r => r.annualizedRate >= parseFloat(minApr as string));
    }
    if (maxApr !== undefined) {
      rates = rates.filter(r => r.annualizedRate <= parseFloat(maxApr as string));
    }
    ...
    // Apply limit
    if (limit) {
      rates = rates.slice(0, parseInt(limit as string));
    }
    ...
    logger.error('[FundingAPI] Failed to get rates:', error);
    res.status(500).json({
      success: false,
...
router.get('/opportunities', async (req, res) => {
  try {
    ...
    const opportunities = await fundingArbitrageScanner.identifyOpportunities(
      parseFloat(threshold as string) / 100
    );
    
    const limited = opportunities.slice(0, parseInt(limit as string));
    ...
    logger.error('[FundingAPI] Failed to get opportunities:', error);
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    });
...
router.get('/history/:symbol', async (req, res) => {
  try {
    ...
    const history = await fundingArbitrageScanner.getFundingHistory(
      symbol.toUpperCase(),
    ...
    logger.error(`[FundingAPI] Failed to get history for ${req.params.symbol}:`, error);
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    });
...
router.get('/stats', async (req, res) => {
  try {
    ...
    logger.error('[FundingAPI] Failed to get stats:', error);
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    });
...
router.get('/compare', async (req, res) => {
  try {
    ...
    logger.error('[FundingAPI] Failed to compare assets:', error);
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    });
...
router.post('/refresh', async (req, res) => {
  try {
    ...
    logger.info('[FundingAPI] Manual refresh triggered');
    ...
    logger.error('[FundingAPI] Refresh failed:', error);
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    });
...
router.get('/alert', async (req, res) => {
  try {
    ...
    const minApr = parseFloat(threshold as string);
    
    const rates = await fundingArbitrageScanner.getAllCurrentRates();
    
    const alerts = rates.filter(r => 
    ...
    logger.error('[FundingAPI] Alert check failed:', error);
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    });
...
router.get('/cross-exchange', async (req, res) => {
  try {
    ...
    const minSpreadValue = parseFloat(minSpread as string);
    const staleThresholdMs = 3 * 60 * 1000;
    
    let opportunities = await fundingArbitrageScanner.getCrossExchangeOpportunities(minSpreadValue);
    ...
    if (forceRefresh || opportunities.length === 0 || isStale) {
      logger.info(
        `[FundingAPI] Running live cross-exchange scan (force=${forceRefresh}, empty=${opportunities.length === 0}, stale=${isStale})`
      );
      await fundingArbitrageScanner.scanCrossExchangeArbitrage();
      opportunities = await fundingArbitrageScanner.getCrossExchangeOpportunities(minSpreadValue);
    }
    ...
    logger.error('[FundingAPI] Failed to get cross-exchange opportunities:', error);
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    });
...
router.get('/cross-exchange/stats', async (req, res) => {
  try {
    ...
    const stats = await fundingArbitrageScanner.getCrossExchangeStats();
    ...
    logger.error('[FundingAPI] Failed to get cross-exchange stats:', error);
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    });
...
router.get('/cross-exchange/:symbol', async (req, res) => {
  try {
    ...
    const opportunity = await fundingArbitrageScanner.getCrossExchangeOpportunity(
      symbol.toUpperCase()
    );
    ...
    logger.error(`[FundingAPI] Failed to get cross-exchange opportunity for ${req.params.symbol}:`, error);
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    });
...
router.get('/exchanges', async (req, res) => {
  try {
    ...
    const exchanges = await fundingArbitrageScanner.getExchangeInfo();
    ...
    logger.error('[FundingAPI] Failed to get exchange info:', error);
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    });
...
router.post('/cross-exchange/scan', async (req, res) => {
  try {
    ...
    logger.info('[FundingAPI] Manual cross-exchange scan triggered');
    
    const opportunities = await fundingArbitrageScanner.scanCrossExchangeArbitrage();
    ...
    logger.error('[FundingAPI] Cross-exchange scan failed:', error);
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    });
...
router.post('/refresh-all', async (req, res) => {
  try {
    ...
    logger.info('[FundingAPI] Complete refresh triggered');
    
    const result = await fundingArbitrageScanner.runCompleteScan();
    ...
    logger.error('[FundingAPI] Complete refresh failed:', error);
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    });
...
router.get('/hyperliquid/live', async (req, res) => {
  try {
    ...
    logger.info('[FundingAPI] Fetching live Hyperliquid funding rates...');
    ...
    if (category) {
      ...
      filtered = categories[category as string] || [];
    }
    ...
    const rates = limited.map(m => ({
      ...
      category: hyperliquidAllMarkets.getMarketsByCategory([m]),
    }));
    ...
    logger.error('[FundingAPI] Failed to get live Hyperliquid rates:', error);
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    });
...
router.get('/hyperliquid/extreme', async (req, res) => {
  try {
    ...
    const minThreshold = parseFloat(threshold as string);
    
    logger.info('[FundingAPI] Fetching extreme funding opportunities...');
    
    const { positive, negative } = await hyperliquidAllMarkets.getExtremeFundingMarkets(minThreshold);
    
    // Format the response
    const formatMarket = (m: any, type: 'long' | 'short') => ({
      symbol: m.coin,
      fundingRate: m.fundingRate,
    ...
    res.json({
      success: true,
      longOpportunities: negative.slice(0, parseInt(limit as string)).map(m => formatMarket(m, 'long')),
      shortOpportunities: positive.slice(0, parseInt(limit as string)).map(m => formatMarket(m, 'short')),
      timestamp: Date.now(),
    });
  } catch (error) {
    logger.error('[FundingAPI] Failed to get extreme funding rates:', error);
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    });
...
router.get('/hyperliquid/categories', async (req, res) => {
  try {
    logger.info('[FundingAPI] Fetching markets by category...');
    
    const { markets } = await hyperliquidAllMarkets.fetchAllMarkets();
    const categories = hyperliquidAllMarkets.getMarketsByCategory(markets);
    
    // Count per category and get top markets
    const result: Record<string, { count: number; topMarkets: any[] }> = {};
    ...
    logger.error('[FundingAPI] Failed to get categories:', error);
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error',
    });
