...
class HeatPredictor {
     ...
     */
    static predictHeat(clusterId, heatHistory, config = {}) {
        const fullConfig = { ...HeatPredictor.DEFAULT_CONFIG, ...config };
        if (heatHistory.length < fullConfig.windowSize) {
            logger_1.default.debug(`[HeatPredictor] Insufficient history for cluster ${clusterId}: ${heatHistory.length} < ${fullConfig.windowSize}`);
            return null;
        }
        // Use recent window
        ...
        const predictions = fullConfig.forecastHorizon.map(hoursAhead => {
            const result = HeatPredictor.predictAtHorizon(heats, hoursAhead, factors, fullConfig);
            return {
                hoursAhead,
     ...
     */
    static calculateLinearTrend(heats) {
     ...
     */
    static determineLifecycleStage(heats) {
     ...
     */
    static predictAtHorizon(heats, hoursAhead, factors, config) {
     ...
     */
    static determineTrajectory(predictions, factors) {
     ...
     */
    static batchPredict(heatHistories, config = {}) {
        const predictions = [];
     ...
     */
    static generateSummary(prediction) {
     ...
     */
    static findPredictedSpikes(predictions, threshold = 0.3) {
     ...
     */
    static findPredictedCrashes(predictions, threshold = -0.3) {
        return predictions.filter(p => {
            const p1h = p.predictions.find(pr => pr.hoursAhead === 1);
            return p1h ? (p1h.predictedHeat - p.currentHeat) / p.currentHeat <= threshold : false;
        });
...
//# sourceMappingURL=heat-predictor.js.map