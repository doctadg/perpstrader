...
exports.getQueryTools = getQueryTools;
exports.getQueryTool = getQueryTool;
exports.getQueryToolDefinitions = getQueryToolDefinitions;
    ...
    return new tools_1.DynamicTool({
        ...
        func: async (input) => {
            ...
            try {
                ...
                logger_1.default.error(`[QueryTool] ${def.name} error:`, error);
...
const getTrace = {
    ...
    handler: async ({ traceId }) => {
        ...
        const trace = trace_store_1.default.getTraceById(traceId);
        if (!trace) {
...
const getRecentTraces = {
    ...
    riskLevel: 'NONE',
    requiresConfirmation: false,
    handler: async ({ limit = 20, agentType }) => {
        trace_store_1.default.initialize();
        const summaries = trace_store_1.default.getRecentTraceSummaries(limit, agentType);
...
const analyzeTrace = {
    ...
    handler: async ({ traceId }) => {
        ...
        try {
            ...
            logger_1.default.error(`[analyzeTrace] Error:`, e);
...
const getNews = {
    ...
    riskLevel: 'NONE',
    requiresConfirmation: false,
    handler: async ({ limit = 25, category }) => {
        if (category) {
            return await news_store_1.default.getNewsByCategory(category, limit);
        }
        return await news_store_1.default.getRecentNews(limit);
    },
};
/**
 * Get hot/trending news clusters
 */
const getHotClusters = {
    ...
    riskLevel: 'NONE',
    requiresConfirmation: false,
    handler: async ({ limit = 25, hours = 24 }) => {
        return await story_cluster_store_1.default.getHotClusters(limit, hours);
    },
};
// ============================================================================
 ...
 */
async function generateTraceAnalysis(traceData) {
...
Return your response as JSON with these fields:
{
  ...
  "insights": ["insight 1", "insight 2"]
    ...
    try {
        const response = await glm_service_1.default.generateText(prompt);
        ...
        logger_1.default.error('[generateTraceAnalysis] Error:', error);
        return {
 ...
 */
function getQueryTool(name) {
    return QUERY_TOOLS[name];
}
/**
 * Get all query tool definitions
 */
function getQueryToolDefinitions() {
    return { ...QUERY_TOOLS };
}
exports.default = QUERY_TOOLS;
