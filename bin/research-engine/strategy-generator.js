...
const glm_service_1 = require("../shared/glm-service");
const logger_1 = __importDefault(require("../shared/logger"));
const uuid_1 = require("uuid");
class StrategyGenerator {
    glmService;
    constructor() {
        this.glmService = new glm_service_1.GLMAIService();
    }
    /**
     * Generate strategy ideas based on current market conditions
     */
    async generateIdeas(marketRegime, count = 5) {
        logger_1.default.info(`[StrategyGenerator] Generating ${count} strategy ideas for ${marketRegime.regime} regime`);
        ...
        if (this.glmService.canUseService()) {
            try {
                ...
                logger_1.default.warn('[StrategyGenerator] GLM generation returned empty, using fallback');
            }
            catch (error) {
                logger_1.default.error('[StrategyGenerator] GLM generation failed:', error);
            }
        }
        else {
            logger_1.default.warn('[StrategyGenerator] GLM service not available, using fallback strategies');
        }
     ...
     */
    async generateWithGLM(marketRegime, count) {
        ...
        try {
            ...
            logger_1.default.error('[StrategyGenerator] GLM API call failed:', error);
            return [];
        }
    }
    /**
     ...
     */
    buildStrategyPrompt(marketRegime, count) {
...
Return valid JSON only:
{
  "strategies": [
     ...
     */
    parseStrategyResponse(response, marketRegime) {
        try {
            ...
            logger_1.default.error('[StrategyGenerator] Failed to parse GLM response:', error);
            return [];
        }
    }
    /**
     ...
     */
    generateFallbackIdeas(marketRegime, count) {
        const ideas = [];
     ...
     */
    validateStrategyType(type) {
        const validTypes = ['TREND_FOLLOWING', 'MEAN_REVERSION', 'MARKET_MAKING', 'ARBITRAGE', 'AI_PREDICTION'];
        return validTypes.includes(type) ? type : 'TREND_FOLLOWING';
    }
}
exports.StrategyGenerator = StrategyGenerator;
exports.default = StrategyGenerator;
//# sourceMappingURL=strategy-generator.js.map