...
class MarketIngester {
    ...
    cleanupOldMarketData(retentionHours) {
        try {
            if (!this.db)
                return;
            ...
            if (result.changes > 0) {
                logger_1.default.info(`[MarketIngester] Cleaned up ${result.changes} old market_data rows (cutoff: ${cutoff})`);
            }
        }
        catch (error) {
            logger_1.default.warn(`[MarketIngester] market_data cleanup failed: ${error?.message}`);
        }
    }
    maybeFlush() {
    ...
    saveOrderBook(snapshot) {
        ...
        if (Number.isFinite(snapshot.midPrice) && snapshot.midPrice > 0) {
            message_bus_1.default.publish(message_bus_1.Channel.MARKET_DATA, {
                symbol,
                price: snapshot.midPrice,
                timestamp: new Date(),
            }).catch(() => { });
        }
        message_bus_1.default.publish(message_bus_1.Channel.ORDER_BOOK_UPDATE, {
            symbol,
            midPrice: snapshot.midPrice,
