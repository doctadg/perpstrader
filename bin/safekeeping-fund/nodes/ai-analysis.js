...
exports.aiAnalysisNode = aiAnalysisNode;
exports.calculateRebalanceConfidence = calculateRebalanceConfidence;
const logger_1 = __importDefault(require("../../shared/logger"));
const axios_1 = __importDefault(require("axios"));
const config = global.config || require('../../shared/config').default?.get?.() || {};
/**
 * AI Analysis Node
 * Leverages OpenRouter to analyze market conditions and provide recommendations
 */
async function aiAnalysisNode(state) {
    logger_1.default.info('[AIAnalysis] Starting AI-powered market analysis');
    try {
        // Check if OpenRouter is available via config
        const apiKey = config.openrouter?.apiKey;
        if (!apiKey) {
            logger_1.default.warn('[AIAnalysis] OpenRouter not available, skipping AI analysis');
            return {
        ...
        logger_1.default.info(`[AIAnalysis] Analysis complete. Risk: ${analysis.riskLevel}, ` +
        ...
        logger_1.default.error(`[AIAnalysis] Failed: ${error}`);
        return {
 ...
 */
async function performAIAnalysis(state) {
    const apiKey = config.openrouter?.apiKey;
    if (!apiKey) {
    ...
    try {
        ...
        logger_1.default.debug(`[AIAnalysis] OpenRouter request failed: ${error}`);
        return getFallbackAnalysis();
    }
}
/**
...
function parseAIResponse(response) {
    try {
        ...
        logger_1.default.error(`[AIAnalysis] Failed to parse response: ${error}`);
        return getFallbackAnalysis();
    }
}
/**
...
function validateRiskLevel(value) {
    ...
    return valid.includes(value?.toUpperCase()) ? value.toUpperCase() : 'MEDIUM';
}
/**
 * Validate market regime
 */
function validateMarketRegime(value) {
    const valid = ['BULL', 'BEAR', 'SIDEWAYS', 'VOLATILE'];
    return valid.includes(value?.toUpperCase()) ? value.toUpperCase() : 'SIDEWAYS';
}
/**
 ...
 */
function calculateRebalanceConfidence(state) {
    let confidence = 0.5; // Base confidence
    // Higher confidence with more opportunities
    if (state.poolOpportunities.length >= 10)
        confidence += 0.1;
    if (state.poolOpportunities.length >= 20)
        confidence += 0.1;
