...
exports.buildAddLiquidityParams = buildAddLiquidityParams;
exports.buildRemoveLiquidityParams = buildRemoveLiquidityParams;
 ...
 */
async function executeNode(state, walletManager) {
    logger_1.default.info('[Execute] Executing rebalance action');
    const startTime = Date.now();
    try {
        if (!state.selectedRebalance) {
            logger_1.default.warn('[Execute] No rebalance action selected');
            return {
        ...
        if (allSuccessful) {
            logger_1.default.info(`[Execute] Execution successful in ${Date.now() - startTime}ms`);
            circuit_breaker_1.default.resetBreaker('safekeeping-execution');
        }
        else {
            logger_1.default.warn(`[Execute] Some executions failed`);
        ...
        logger_1.default.error(`[Execute] Failed: ${error}`);
        // Note: onError is private, handled at orchestrator level
        return {
 ...
 */
async function executeMint(action, walletManager) {
    const startTime = Date.now();
    if (!action.toPool) {
        return {
            actionId: action.id,
    ...
    logger_1.default.info(`[Execute] Minting position in ${action.toPool.address}`);
    const params = {
 ...
 */
async function executeBurn(action, walletManager) {
    const startTime = Date.now();
    if (!action.fromPool) {
        return {
            actionId: action.id,
    ...
    logger_1.default.info(`[Execute] Burning position from ${action.fromPool.address}`);
    const params = {
 ...
 */
async function executeReallocate(action, walletManager) {
    const results = [];
    ...
    logger_1.default.info(`[Execute] Reallocating ${action.percentage * 100}% from ${action.fromPool.address} ` +
 ...
 */
async function executeCollect(action, walletManager) {
    const startTime = Date.now();
    logger_1.default.info(`[Execute] Collecting fees from ${action.toPool?.address || 'unknown'}`);
 ...
 */
function buildAddLiquidityParams(action, token0Amount, token1Amount) {
    return {
        poolAddress: action.toPool?.address || '',
        token0Amount,
        token1Amount,
 ...
 */
function buildRemoveLiquidityParams(action) {
    return {
        positionId: action.fromPool?.address || '',
        percentage: action.percentage * 100,
        slippageTolerance: 0.005,
