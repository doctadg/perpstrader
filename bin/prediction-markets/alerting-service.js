...
class AlertingService {
     ...
     */
    async tradeExecuted(trade, portfolio) {
...
**Available:** $${portfolio.availableBalance.toFixed(2)}
     ...
     */
    async stopLossTriggered(position, exitPrice, pnl) {
        const message = {
            type: 'STOP_LOSS',
...
**P&L:** $${pnl.toFixed(2)}
     ...
     */
    async emergencyStop(reason, portfolio) {
        const message = {
            type: 'EMERGENCY_STOP',
...
**Time:** ${new Date().toISOString()}
     ...
     * Daily P&L report
     */
    async dailyPnLReport(dailyPnL, totalTrades, winRate, portfolio) {
...
**Open Positions:** ${portfolio.positions.length}
    ...
    async sendAlert(message) {
        ...
        // Log to console
        logger_1.default.info(`[Alert] ${message.title}: ${message.message.substring(0, 100)}...`);
        ...
        try {
            ...
            logger_1.default.error('[AlertingService] Failed to send alert:', error);
        }
    }
    async sendTelegram(message) {
        ...
        try {
            await axios_1.default.post(`https://api.telegram.org/bot${this.config.telegram.botToken}/sendMessage`, {
            ...
            logger_1.default.error('[AlertingService] Telegram send failed:', error);
        }
    }
    async sendDiscord(message) {
        ...
        try {
            await axios_1.default.post(this.config.discord.webhookUrl, {
                embeds: [{
                        title: message.title,
            ...
            logger_1.default.error('[AlertingService] Discord send failed:', error);
        }
    }
    async sendWebhook(message) {
        if (!this.config.webhook)
            return;
        try {
            await axios_1.default.post(this.config.webhook.url, {
                ...message,
                timestamp: new Date().toISOString(),
            }, {
            ...
            logger_1.default.error('[AlertingService] Webhook send failed:', error);
        }
    }
     ...
     * Get alert history
     */
    getAlertHistory(type, since) {
        return this.alertHistory.filter(a => {
            if (type && a.message.type !== type)
                return false;
            if (since && a.timestamp < since)
                return false;
            return true;
        });
    }
