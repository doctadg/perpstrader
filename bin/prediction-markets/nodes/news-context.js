...
function resolveTrend(clusters) {
    let up = 0;
    let down = 0;
    for (const cluster of clusters) {
        if (cluster.trendDirection === 'UP')
            up += 1;
        else if (cluster.trendDirection === 'DOWN')
            down += 1;
    }
    if (up > down)
        return 'UP';
...
function resolveUrgency(clusters) {
    let best = 'LOW';
    for (const cluster of clusters) {
...
function buildMarketIntel(market, news, clusters) {
...
async function newsContextNode(state) {
    ...
    const articleIds = Object.values(marketNews)
        ...
        .filter(Boolean);
    let articleToCluster = new Map();
    const clustersById = new Map();
    if (articleIds.length) {
        try {
            articleToCluster = await story_cluster_store_1.default.findClusterIdsByArticleIds(articleIds);
            const uniqueClusterIds = Array.from(new Set(Array.from(articleToCluster.values()).filter(Boolean))).slice(0, 250);
            const clusterRows = await Promise.all(uniqueClusterIds.map(async (clusterId) => {
                const cluster = await story_cluster_store_1.default.getClusterById(clusterId);
            ...
            logger_1.default.warn('[PredictionNewsContext] Failed to hydrate cluster heat data:', error);
        }
    }
    for (const market of state.activeMarkets) {
        const news = marketNews[market.id] || [];
        const clusterIds = Array.from(new Set(news
