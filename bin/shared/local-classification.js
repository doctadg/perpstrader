...
exports.classifyArticleLocally = classifyArticleLocally;
exports.classifyBatch = classifyBatch;
exports.enhanceWithLocalClassification = enhanceWithLocalClassification;
const news_trend_1 = require("./news-trend");
...
function classifyArticleLocally(article) {
...
function extractCryptoTickers(text) {
    const tickers = [];
    const knownTickers = [
...
function detectSportsLeague(text) {
...
function detectSubEventType(text) {
...
function detectTrendDirection(text, subEventType) {
    ...
    if (upTypes.includes(subEventType))
        return 'UP';
    if (downTypes.includes(subEventType))
        return 'DOWN';
    // Check keywords
    let upCount = 0;
    let downCount = 0;
    ...
    if (upCount > downCount * 1.3 && upCount > 1)
        return 'UP';
    if (downCount > upCount * 1.3 && downCount > 1)
        return 'DOWN';
    return 'NEUTRAL';
}
function detectUrgency(text, importance) {
...
function buildSpecificTopic(title, tickers, subEventType, sportsLeague) {
    ...
    if (topic.length < 10) {
        return (0, news_trend_1.deriveTrend)({ title, tags: [] }).topic;
    }
    return topic;
}
function calculateConfidence(title, subEventType, trendDirection) {
    let confidence = 0.5;
...
function enhanceWithLocalClassification(article, derivedTopic, derivedKeywords) {
...
//# sourceMappingURL=local-classification.js.map