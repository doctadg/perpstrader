...
Return JSON ONLY:
{
  ...
  "keywords": ["...", "..."]
     ...
     */
    async processEventLabelBatch(batch, batchIndex) {
        const results = new Map();
        // Check cache for all items in batch first
        const uncached = [];
        for (const item of batch) {
            const fingerprint = (0, title_cleaner_1.getTitleFingerprint)(item.title);
            const cached = await redis_cache_1.default.getEventLabel(fingerprint);
            if (cached) {
                this.cacheHits++;
        ...
        if (uncached.length === 0) {
            logger_1.default.info(`[OpenRouter] Batch ${batchIndex}: All ${batch.length} items from cache`);
            return results;
        }
        logger_1.default.info(`[OpenRouter] Batch ${batchIndex}: Cache hit ${batch.length - uncached.length}/${batch.length}, LLM processing ${uncached.length}`);
  ...
  "labels": [
        ...
        try {
            ...
            if (jsonMatch) {
                try {
                        ...
                        if (label.id && label.topic && label.topic.length > 5) {
                            const result = {
                                ...
                                keywords: Array.isArray(label.keywords)
                                    ? label.keywords.map((k) => String(k).trim()).filter(Boolean).slice(0, 7)
                                    : [],
                            };
                            ...
                            if (originalItem) {
                                const fingerprint = (0, title_cleaner_1.getTitleFingerprint)(originalItem.title);
                                await redis_cache_1.default.setEventLabel(fingerprint, result);
                            }
                        }
                    }
                    ...
                    logger_1.default.warn(`[OpenRouter] Batch ${batchIndex} JSON parse failed: ${parseError}`);
                    ...
                    try {
                        ...
                        logger_1.default.debug(`[OpenRouter] Emergency extraction also failed: ${fallbackError}`);
                    }
                }
            }
            else {
                logger_1.default.warn(`[OpenRouter] Batch ${batchIndex}: No JSON found in response. Length: ${content.length}, Preview: ${content.substring(0, 200)}...`);
            }
    ...
    async batchEventLabels(inputs) {
        ...
        for (let i = 0; i < batches.length; i += concurrency) {
            ...
            const batchResults = await Promise.all(concurrentBatches.map((batch, idx) => this.processEventLabelBatch(batch, i + idx + 1)));
            for (const batchResult of batchResults) {
                for (const [id, label] of batchResult) {
                    results.set(id, label);
                }
            }
            logger_1.default.info(`[OpenRouter] Completed ${Math.min(i + concurrency, batches.length)}/${batches.length} batches, ${results.size} labeled`);
        }
        return results;
    }
    /**
...
Return JSON in this format:
{
  "articles": [
     ...
     */
    emergencyExtractLabels(content, uncached, results) {
        ...
        if (extracted > 0) {
            logger_1.default.info(`[OpenRouter] Emergency extraction salvaged ${extracted} labels`);
        }
    }
}
