...
function extractKeyTerms(title, action) {
    ...
    const keyTerms = words.filter(w => !STOP_WORDS.has(w) &&
        !actionVariants.has(w) &&
        !/^\d+$/.test(w));
    return [...new Set(keyTerms)].slice(0, 4);
}
function determineTrend(action, keywords) {
    ...
    if (upActions.has(lowerAction))
        return 'UP';
    if (downActions.has(lowerAction))
        return 'DOWN';
    const lowerKeywords = keywords.join(' ').toLowerCase();
    if (/surge|rally|gain|up|record|high|boost|growth|profit|beat/i.test(lowerKeywords))
        return 'UP';
    if (/crash|plunge|drop|fall|loss|down|low|crash|ban|hack|seiz/i.test(lowerKeywords))
        return 'DOWN';
    return 'NEUTRAL';
}
function generateTopic(title) {
    ...
    if (action && keyTerms.length > 0) {
        ...
        const fallback = title.split(/\s+/).slice(0, 6).map(w => w.replace(/[^a-zA-Z0-9-]/g, '')).filter(w => w.length > 2).map(w => toTitleCaseWord(w)).join(' ');
