...
class NewsHeatmapService {
    ...
    configuredLabelingModel;
    llmBlockedUntil = 0;
    llmConsecutiveEmpty = 0;
    constructor() {
    ...
    calculateArticleWeight(article, nowMs) {
    ...
    chooseKeywords(acc) {
        ...
        for (const [k, v] of acc.keywordWeights)
            weightedKeywords.set(k, (weightedKeywords.get(k) || 0) + v * 1.35);
        for (const [k, v] of acc.tokenWeights)
    ...
    resolveTrendDirection(acc, velocity) {
    ...
    resolveUrgency(acc, heatScore) {
        const urgencyRanking = ['LOW', 'MEDIUM', 'HIGH', 'CRITICAL'];
    ...
    stableFallbackKey(acc) {
        ...
        if (topTokens.length === 0) {
            const fallbackHash = crypto_1.default.createHash('sha1').update(acc.articles[0]?.article.id || crypto_1.default.randomUUID()).digest('hex').slice(0, 16);
            return `${acc.category}:cluster:${fallbackHash}`;
        }
        return `${acc.category}:${topTokens.join('|')}`;
    }
    createAccumulator(key, category, firstArticle) {
        return {
            key,
            category,
            topicVotes: new Map(),
    ...
    addArticleToAccumulator(acc, article, tokenSet, weight, label) {
    ...
    findBestCluster(clusters, category, tokenSet, label) {
        let best = null;
        let bestScore = 0;
    ...
    mergeAccumulators(target, source) {
        for (const article of source.articles)
            target.articles.push(article);
        for (const sourceName of source.sourceSet)
    ...
    async getRecentArticles(hours, limit) {
    ...
    `).all(cutoff, cutoff, limit);
        return rows
            ...
            const categoryList = categories.length > 0 ? categories : ['CRYPTO'];
            const tags = this.parseJsonArray(row.tags).slice(0, 12);
            const sentiment = ['BULLISH', 'BEARISH', 'NEUTRAL'].includes(row.sentiment)
                ? row.sentiment
                : 'NEUTRAL';
            const importance = ['LOW', 'MEDIUM', 'HIGH', 'CRITICAL'].includes(row.importance)
                ? row.importance
                : 'MEDIUM';
            return {
                id: String(row.id),
    ...
    async labelArticlesWithLlm(articles) {
        ...
        if (labelInputs.length === 0)
            return labels;
        try {
            const llmLabels = await this.withTimeout(openrouter_service_1.default.batchEventLabels(labelInputs), this.llmTimeoutMs, `LLM labeling timed out after ${this.llmTimeoutMs}ms`);
            for (const [id, rawLabel] of llmLabels) {
                const topic = String(rawLabel?.topic || '').trim();
                if (!topic)
                    continue;
                const trendDirection = ['UP', 'DOWN', 'NEUTRAL'].includes(rawLabel?.trendDirection)
                    ? rawLabel.trendDirection
                    : 'NEUTRAL';
                const urgency = ['CRITICAL', 'HIGH', 'MEDIUM', 'LOW'].includes(rawLabel?.urgency)
                    ? rawLabel.urgency
                    : 'MEDIUM';
                const keywords = Array.isArray(rawLabel?.keywords)
                    ? rawLabel.keywords.map((keyword) => String(keyword)).filter(Boolean).slice(0, 8)
                    : [];
            ...
            logger_1.default.warn('[NewsHeatmapService] LLM labeling failed, continuing with lexical fallback:', error);
        }
        return labels;
    }
    ...
    buildClusters(articles, llmLabels, previousState, now) {
        const nowMs = now.getTime();
        const workingClusters = [];
        const byLlmTopicKey = new Map();
        ...
        for (const article of sortedArticles) {
            ...
            if (!assignedCluster) {
                const seedKey = llmTopicKey || `${category}:seed:${crypto_1.default.randomUUID()}`;
        ...
        const stableClusterMap = new Map();
        ...
        for (const cluster of stableClusterMap.values()) {
            ...
            const topicKey = this.normalizeTopicKey(topic) || crypto_1.default.createHash('sha1').update(cluster.key).digest('hex').slice(0, 24);
            ...
            const clusterId = previous?.clusterId || `nh_${crypto_1.default.createHash('sha1').update(stableKey).digest('hex').slice(0, 18)}`;
    ...
    async persistState(clusters, timestamp) {
        await this.initialize();
    ...
    buildCacheKey(hours, category, articleLimit) {
        return `${hours}:${category}:${articleLimit}`;
    }
    projectResult(raw, limit) {
    ...
    async buildInternal(options) {
        ...
        const llmCoverage = articles.length > 0
            ...
            : 0;
        const llmEnabled = openrouter_service_1.default.canUseService() && Date.now() >= this.llmBlockedUntil;
        return {
    ...
    async getClusterDetails(clusterId, hours = 48) {
        if (!clusterId)
            return null;
    ...
    async getTimeline(hours = 24, bucketHours = 2, category = 'ALL') {
        await this.initialize();
        if (!this.db) {
            return {
...
//# sourceMappingURL=news-heatmap-service.js.map