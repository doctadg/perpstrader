...
exports.defiLlamaYields = void 0;
...
class DefiLlamaYields {
    ...
    async fetchPools() {
        ...
        if (cached && Date.now() - cached.timestamp < this.CACHE_TTL) {
            logger_1.default.debug('[DefiLlama] Returning cached pool data');
            return cached.pools;
        }
        try {
            logger_1.default.info('[DefiLlama] Fetching pool yields from DefiLlama...');
            const response = await axios_1.default.get(DEFILLAMA_POOLS_URL, {
                timeout: 30_000,
                headers: {
            ...
            if (!data || !data.data) {
                logger_1.default.warn('[DefiLlama] No data in response');
                return cached?.pools || [];
            }
            ...
            logger_1.default.info(`[DefiLlama] Found ${filtered.length} pools across target DEXs ` +
                `(from ${rawPools.length} total)`);
            // Filter by minimum TVL and volume
            const qualified = filtered.filter(pool => pool.tvlUsd >= MIN_TVL && pool.volumeUsd1d >= MIN_VOLUME_24H);
            logger_1.default.info(`[DefiLlama] ${qualified.length} pools meet TVL/volume thresholds`);
            ...
            for (const opp of top5) {
                logger_1.default.info(`[DefiLlama] ${opp.dex} ${opp.chain}: ${opp.token0.symbol}/${opp.token1.symbol} ` +
            ...
            logger_1.default.error(`[DefiLlama] Failed to fetch pools: ${error}`);
            // Return stale cache if available
            if (cached) {
                logger_1.default.warn('[DefiLlama] Using stale cache data');
                return cached.pools;
            }
            return [];
        }
     ...
     */
    async fetchStablePools() {
        const allPools = await this.fetchPools();
        const stablecoins = ['USDC', 'USDT', 'DAI', 'FDUSD', 'BUSD', 'TUSD'];
        return allPools.filter(p => stablecoins.includes(p.token0.symbol) && stablecoins.includes(p.token1.symbol));
    }
    /**
    ...
    async getSummary() {
        ...
        for (const pool of pools) {
            ...
            if (pool.effectiveAPR > bestAPR)
                bestAPR = pool.effectiveAPR;
            // By DEX
            if (!byDEX[pool.dex]) {
    ...
    estimateFeeTier(pool, dex) {
        ...
        if (pool.tvlUsd > 1_000_000 && pool.apy < 10)
            return 100; // 0.01%
        if (pool.tvlUsd > 500_000 && pool.apy < 20)
            return 500; // 0.05%
        if (pool.tvlUsd > 100_000)
            return dex === 'pancakeswap_v3' ? 2500 : 3000; // 0.25% or 0.3%
        return 10000; // 1%
    }
    /**
    ...
    calculateRiskScore(pool) {
        ...
        if (pool.ilRisk === 'none')
            risk -= 0.1;
        else if (pool.ilRisk === 'high')
            risk += 0.2;
        else if (pool.ilRisk === 'medium')
            risk += 0.1;
        ...
        if (pool.stablecoin)
            risk -= 0.15;
        // Confidence level
        if (pool.confidence?.level === 'high')
            risk -= 0.05;
        else if (pool.confidence?.level === 'low')
            risk += 0.1;
        // Category
        if (pool.category === 'Stableswap')
            risk -= 0.1;
...
// Singleton export
exports.defiLlamaYields = new DefiLlamaYields();
exports.default = exports.defiLlamaYields;
//# sourceMappingURL=defillama-yields.js.map