...
exports.Selector = exports.DEFAULT_SELECTION_CONFIG = void 0;
exports.DEFAULT_SELECTION_CONFIG = {
...
class Selector {
    config;
    constructor(config = {}) {
        this.config = { ...exports.DEFAULT_SELECTION_CONFIG, ...config };
    }
    /**
     ...
     */
    selectNextGeneration(population, targetSize) {
        ...
        const diversitySelections = this.selectDiverseGenomes(sorted.slice(this.config.eliteCount), diversitySlots);
     ...
     */
    selectByMethod(candidates, count) {
     ...
     */
    tournamentSelection(candidates, count) {
        const selected = [];
     ...
     */
    rouletteSelection(candidates, count) {
        ...
        if (totalFitness === 0) {
            ...
            return Array.from({ length: count }, () => candidates[Math.floor(Math.random() * candidates.length)]);
        }
        const selected = [];
        while (selected.length < count) {
     ...
     */
    rankSelection(candidates, count) {
     ...
     */
    truncationSelection(candidates, count) {
        const sorted = [...candidates].sort((a, b) => (b.fitness || 0) - (a.fitness || 0));
        const cutoff = Math.ceil(candidates.length * 0.5); // Top 50%
        const topCandidates = sorted.slice(0, cutoff);
        return Array.from({ length: count }, () => topCandidates[Math.floor(Math.random() * topCandidates.length)]);
    }
    /**
     * Select diverse genomes using crowding distance
     */
    selectDiverseGenomes(candidates, count) {
     ...
     */
    checkConvergence(population, threshold = 0.05) {
