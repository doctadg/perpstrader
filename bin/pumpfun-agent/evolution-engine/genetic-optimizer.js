...
class GeneticOptimizer {
    ...
    `);
        stmt.run(genome.id, genome.strategyId || null, genome.parentIds && genome.parentIds.length > 0 ? genome.parentIds.join(',') : null, genome.generation, JSON.stringify(genome.parameters), genome.mutationType || null, genome.fitness, genome.sharpeRatio, genome.totalReturn, genome.maxDrawdown, genome.winRate, genome.totalTrades, genome.backtestResult ? JSON.stringify(genome.backtestResult) : null, genome.createdAt.toISOString());
    }
    /**
