        ...
        try {
            ...
            try {
                ...
                for (const { perf, idea, paramsHash } of batchEntries) {
                    ...
                    // Check if strategy already exists in strategies table (match by name + params hash)
                    const existing = db.prepare('SELECT id, params_hash FROM strategies WHERE name = ? AND (params_hash = ? OR params_hash IS NULL AND type = ?)').get(idea.name, paramsHash, idea.type);
                    if (existing) {
