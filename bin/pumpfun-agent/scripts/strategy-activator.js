...
function parseArgs() {
    ...
    for (let i = 0; i < args.length; i++) {
        switch (args[i]) {
            case '--top':
                topN = parseInt(args[++i]) || DEFAULT_TOP_N;
                break;
            case '--min-trades':
                minTrades = parseInt(args[++i]) || DEFAULT_MIN_TRADES;
                break;
            case '--min-sharpe':
                minSharpe = parseFloat(args[++i]) || DEFAULT_MIN_SHARPE;
                break;
            case '--max-drawdown':
                maxDrawdown = parseFloat(args[++i]) || DEFAULT_MAX_DRAWDOWN;
                break;
            case '--dry-run':
                dryRun = true;
                break;
 ...
 */
function calculateCompositeScore(sharpe, winRate, pnl, drawdown, trades) {
...
function main() {
    ...
    try {
        ...
        const totalStrategies = db.prepare('SELECT COUNT(*) as c FROM strategies').get().c;
        const activeStrategies = db.prepare('SELECT COUNT(*) as c FROM strategies WHERE isActive = 1').get().c;
        const totalIdeas = db.prepare('SELECT COUNT(*) as c FROM strategy_ideas').get().c;
        const totalPerformance = db.prepare('SELECT COUNT(*) as c FROM strategy_performance').get().c;
    ...
    `).all(config.minTrades, config.minSharpe, config.maxDrawdown);
    ...
    `).all(config.minTrades, config.minSharpe, config.maxDrawdown);
        ...
        const selected = [];
        const usedKeys = new Set(); // Track unique (type, symbols_hash) combos
        for (const candidate of candidates) {
            if (selected.length >= config.topN)
                break;
        ...
        if (config.dryRun) {
            ...
            for (const s of selected) {
                if (s.source === 'strategies') {
                    ...
                    const idea = db.prepare('SELECT * FROM strategy_ideas WHERE id = ?').get(s.id);
                    if (idea) {
                        const perf = db.prepare('SELECT * FROM strategy_performance WHERE strategy_id = ?').get(s.id);
            ...
            // Verify
            const newActive = db.prepare('SELECT COUNT(*) as c FROM strategies WHERE isActive = 1').get().c;
            const newInactive = db.prepare('SELECT COUNT(*) as c FROM strategies WHERE isActive = 0').get().c;
        ...
        const zeroDdCount = db.prepare("SELECT COUNT(*) as c FROM strategies WHERE json_extract(performance, '$.maxDrawdown') = 0 AND json_extract(performance, '$.totalTrades') > 0").get().c;
        if (zeroDdCount > 0) {
        ...
        const highSharpeCount = db.prepare("SELECT COUNT(*) as c FROM strategies WHERE json_extract(performance, '$.sharpeRatio') > 100").get().c;
        ...
        const lowTradeCount = db.prepare("SELECT COUNT(*) as c FROM strategies WHERE json_extract(performance, '$.totalTrades') < 5").get().c;
        if (lowTradeCount > 0) {
...
main();
//# sourceMappingURL=strategy-activator.js.map