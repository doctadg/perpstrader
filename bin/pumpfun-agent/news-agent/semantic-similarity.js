...
const local_embeddings_1 = require("../shared/local-embeddings");
const enhanced_entity_extraction_1 = require("./enhanced-entity-extraction");
class SemanticSimilarityService {
    ...
    async calculateSimilarity(article1, article2) {
        ...
        let llmSim;
        if (this.useLLM && openrouter_service_1.default.canUseService()) {
        ...
        const confidence = this.calculateConfidence(features1.entities.length, features2.entities.length, llmSim !== undefined);
        return {
     ...
     */
    async batchCalculateSimilarity(article, candidates) {
        const results = [];
     ...
     */
    async findMostSimilar(article, candidates, topK = 5, threshold = 0.60) {
    ...
    async extractFeatures(article) {
        ...
        // Extract entities
        const entityResult = await enhanced_entity_extraction_1.enhancedEntityExtractor.extractHybrid(article.title, article.content, article.id);
    ...
    async generateEmbedding(text) {
        ...
        if (openrouter_service_1.default.canUseService()) {
            try {
                const openrouterEmbedding = await openrouter_service_1.default.generateEmbedding(text);
        ...
        return (0, local_embeddings_1.embedText)(text, this.embeddingDim);
    }
    /**
     * Extract topic and keywords from article
     */
    extractTopicAndKeywords(title, content) {
    ...
    calculateEntitySimilarity(entities1, entities2) {
        ...
        for (const e1 of entities1) {
            ...
            const match = entities2.find(e2 => e2.normalized === e1.normalized ||
     ...
     */
    async calculateLLMSimilarity(article1, article2) {
        try {
...
Return only a number from 0-100 representing similarity percentage.`;
            ...
            logger_1.default.debug(`[SemanticSimilarity] LLM similarity failed: ${error}`);
        }
        return undefined;
    }
    /**
     ...
     */
    calculateConfidence(entityCount1, entityCount2, hasLLMResult) {
        let confidence = 0.5;
     ...
     */
    getCachedEmbedding(id) {
        return this.embeddingCache.get(id);
    }
    /**
     * Cache embedding with LRU eviction
     */
    cacheEmbedding(id, embedding) {
    ...
    clearCache() {
        ...
        logger_1.default.info('[SemanticSimilarity] Cache cleared');
    }
    /**
     * Get cache stats
     */
...
exports.default = exports.semanticSimilarityService;
