...
class SocialAnalyzerService {
    ...
    async analyzeSocial(metadata) {
        logger_1.default.debug(`[SocialAnalyzer] Analyzing social links for token`);
        // Analyze Twitter
    ...
    async analyzeTwitter(handle) {
        ...
        try {
            ...
            logger_1.default.debug(`[SocialAnalyzer] Twitter analysis failed for ${handle}: ${error}`);
            return this.getEmptyTwitter();
        }
    }
    /**
     ...
     */
    async analyzeTelegram(username) {
        if (!username) {
        ...
        try {
            ...
            logger_1.default.debug(`[SocialAnalyzer] Telegram analysis failed for ${username}: ${error}`);
            return this.getEmptyTelegram();
        }
    }
    /**
    ...
    async analyzeDiscord(invite) {
        ...
        try {
            ...
            logger_1.default.debug(`[SocialAnalyzer] Discord analysis failed for ${invite}: ${error}`);
            return this.getEmptyDiscord();
        }
    }
    /**
    ...
    calculateOverallScore(social) {
        ...
        if (social.twitter.exists) {
            ...
            if (social.twitter.followerCount > 100)
                score += 0.1;
            if (social.twitter.followerCount > 1000)
                score += 0.1;
            if (social.twitter.followerCount > 10000)
                score += 0.1;
        }
        // Telegram (up to 0.35)
        maxScore += 0.35;
        if (social.telegram.exists) {
            ...
            if (social.telegram.memberCount > 100)
                score += 0.1;
            if (social.telegram.memberCount > 1000)
                score += 0.15;
        }
        // Discord (up to 0.25)
        maxScore += 0.25;
        if (social.discord.exists) {
            ...
            if (social.discord.memberCount > 100)
                score += 0.08;
            if (social.discord.memberCount > 1000)
                score += 0.07;
        }
...
exports.default = socialAnalyzerService;
//# sourceMappingURL=social-analyzer.js.map