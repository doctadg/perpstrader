...
function normalizeStatus(raw) {
    ...
    if (raw?.closed === false && raw?.active === true)
        return 'OPEN';
    ...
    for (const url of urls) {
        try {
            ...
            if (typeof response.data === 'string' && response.data.includes('<html')) {
                logger_1.default.warn(`[Polymarket] HTML response from ${url}, skipping`);
                continue;
            }
            ...
            logger_1.default.info(`[Polymarket] Loaded ${limited.length} latest markets from ${url}`);
            if (limited.length) {
                return limited;
            }
        }
        catch (error) {
            logger_1.default.warn(`[Polymarket] Failed to fetch markets from ${url}:`, error);
        }
    }
    logger_1.default.error('[Polymarket] Failed to fetch markets from all endpoints');
    return [];
}
async function fetchCandles(tokenId) {
    if (!tokenId)
        return [];
    try {
        ...
        logger_1.default.warn(`[Polymarket] History fetch failed for ${tokenId}:`, error.message);
        return [];
    }
}
exports.default = {
