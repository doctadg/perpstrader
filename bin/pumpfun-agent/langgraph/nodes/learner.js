...
async function learnerNode(state) {
    logger_1.default.info(`[LearnerNode] Processing learning for cycle ${state.cycleId}`);
    ...
    try {
        ...
        if (state.candles && state.candles.length >= 20 && state.indicators) {
            ...
            try {
                const patternId = await vector_store_1.default.storePattern(state.symbol, state.timeframe, state.candles, state.indicators, outcome, historicalReturn, state.regime || 'RANGING');
                patternsStored++;
                ...
                errors.push(msg);
                logger_1.default.warn(`[LearnerNode] ${msg}`);
            }
        }
        // Store trade outcome for learning
        if (state.executionResult && state.indicators && state.candles) {
            try {
                await vector_store_1.default.storeTradeOutcome(state.selectedStrategy?.id || 'unknown', state.symbol, state.indicators, state.candles, state.executionResult.pnl || 0, {
        ...
        const stats = await vector_store_1.default.getStats();
        thoughts.push(`Learning complete: ${patternsStored} patterns, ${tradesStored} trades stored`, `Vector store stats: ${stats.patterns} patterns, ${stats.trades} trades total`);
        logger_1.default.info(`[LearnerNode] Stored ${patternsStored} patterns, ${tradesStored} trades`);
        return {
        ...
        logger_1.default.error('[LearnerNode] Learning failed:', error);
        return {
