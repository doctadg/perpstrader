...
class CoinGenerator {
     ...
     */
    async generate(narrative) {
        // 1. Use LLM to generate coin concept
        const llmResult = await this.generateCoinConcept(narrative);
        // 2. Generate token image
        const imageBuffer = await (0, image_generator_1.generateTokenImage)(llmResult.symbol, llmResult.name, {
        ...
        if (this.config.pinataJwt) {
            const { imageUri, metadataUri } = await (0, metadata_builder_1.uploadToIPFS)(imageBuffer, llmResult.name, llmResult.symbol, llmResult.description, { pinataJwt: this.config.pinataJwt });
            metadata = (0, metadata_builder_1.buildMetadata)(llmResult.name, llmResult.symbol, llmResult.description, imageUri);
            metadata.image = imageUri;
        }
        else {
            // No Pinata — return with placeholder URI
            metadata = (0, metadata_builder_1.buildMetadata)(llmResult.name, llmResult.symbol, llmResult.description, 'ipfs://pending-upload');
        }
        return {
            name: llmResult.name,
     ...
     */
    async generateCoinConcept(narrative) {
...
//# sourceMappingURL=coin-generator.js.map